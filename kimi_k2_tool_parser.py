...
class KimiK2ToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: AnyTokenizer):
        ...
        # Original format: <|tool_call_begin|>functions.name:0<|tool_call_argument_begin|>{"args"}<|tool_call_end|>
        self.tool_call_regex = re.compile(
            r"<\|tool_call_begin\|>\s*(?P<tool_call_id>.+:\d+)\s*<\|tool_call_argument_begin\|>\s*(?P<function_arguments>.*?)\s*<\|tool_call_end\|>"
        )
        
        # Alternative format: {"type": "functions.name:1<|tool_call_argument_begin|>{"args"}<|tool_call_end|>
        self.alt_tool_call_regex = re.compile(
            r'\{"type":\s*"(?P<tool_call_id>functions\..+?:\d+)<\|tool_call_argument_begin\|>(?P<function_arguments>.*?)<\|tool_call_end\|>'
        )
    ...
    def extract_tool_calls(
        ...
        # Check for either official format or alternative format
        has_official_format = self.tool_calls_start_token in model_output
        has_alt_format = '{"type": "functions.' in model_output and '<|tool_call_argument_begin|>' in model_output
        
        if not has_official_format and not has_alt_format:
            ...
            try:
                ...
                if not function_call_tuples:
                    function_call_tuples = self.alt_tool_call_regex.findall(
                        model_output)
                    logger.debug("Using alternative regex pattern")
                ...
                for match in function_call_tuples:
                    ...
                    # function_id: functions.get_weather:0 or functions.Task:1
                ...
                if has_official_format:
                    content = model_output[:model_output.find(self.tool_calls_start_token)]
                else:
                    # For alternative format, extract content before first {"type": "functions.
                    alt_start = model_output.find('{"type": "functions.')
                    content = model_output[:alt_start] if alt_start > 0 else None
